...
import { dummyProfile, dummyProfileStatus } from '../utils/lists';
...
const Profile: React.FC = () => {
  ...
  const handleAddSocialLink = async (platform: string, url: string) => {
    ...
    const response = await profileApi.addSocialLink({
      ...
      setProfile((prev) => {
        ...
        return {
          ...
          socialLinks: prev.socialLinks.map((l) =>
            l.id === optimisticLink.id ? response.data! : l
          ),
        };
      });
    } else {
    ...
    try {
      ...
      try {
        ...
        if (result) {
          setProfile((prev) =>
            prev
              ? { ...prev, profilePictureUrl: result.profilePictureUrl }
              : null
          );
        }
      } catch (apiError) {
        ...
        setProfile((prev) =>
          prev ? { ...prev, profilePictureUrl: imageUrl } : null
        );
      }
      await refreshProfileStatus();
    } finally {
  ...
  const handleAddSkill = async (
    ...
    try {
      try {
        const newSkillResponse = await profileApi.addSkill({
          name: skillName,
          proficiency,
        });
        ...
        if (newSkill) {
          setProfile((prev) =>
            prev ? { ...prev, skills: [...prev.skills, newSkill] } : null
          );
        }
      } catch (apiError) {
    ...
    <DashboardLayout
      ...
      actions={
        <div className="flex items-center gap-2 md:gap-4">
          ...
          <Button
            onClick={() => profileApi.exportProfilePdf()}
            variant="outline"
            size="sm"
            className="hidden sm:flex"
          >
              ...
              <div className="rounded-2xl bg-white p-8 shadow-sm border border-gray-100 dark:bg-brand-dark-lighter dark:border-gray-700 relative overflow-hidden">
                ...
                <div className="relative z-10 flex flex-col items-center text-center">
                  <FileUpload
                    ...
                    className="mx-auto w-28 h-28 border-4 border-white dark:border-brand-dark-lighter shadow-md"
                  />
                  ...
                  <div className="flex flex-col gap-2 w-full text-sm text-gray-600 dark:text-gray-300 items-center justify-center">
                    {profile.location && (
                      <span className="flex items-center">
                        <Globe className="w-3 h-3 mr-1" /> {profile.location}
                      </span>
                    )}
                    {profile.phone && (
                      <span className="flex items-center">
                ...
                {isEditingPersonalInfo ? (
                  <div className="space-y-4">
                    ...
                    <textarea
                      ...
                      value={formData.bio}
                      onChange={handleInputChange}
                      className="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm focus:border-brand-orange dark:bg-gray-800 dark:text-gray-100 min-h-[120px]"
              ...
              <div className="rounded-2xl bg-white p-6 shadow-sm border border-gray-100 dark:bg-brand-dark-lighter dark:border-gray-700">
                ...
                >
                  <div className="flex items-center">
                    ...
                    <span className="ml-3 font-semibold text-gray-700 dark:text-gray-200 group-hover:text-brand-orange transition-colors">
                      Update Password
                    </span>
                  </div>
                  <ArrowLeft
                    className={`w-5 h-5 text-gray-400 transition-transform duration-200 ${
                      showPasswordSection ? '-rotate-90' : 'rotate-180'
                    }`}
                  />
                </button>
                {showPasswordSection && (
